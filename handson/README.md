...
このディレクトリは、講座内で配布するハンズオン素材です。学習者は、AWSリソースを作る前にこのREADMEで目的、作成物、料金注意、削除手順を確認します。
...
- AWSリソースを作成するハンズオンは、AWS CloudShellで実行することを標準にします。
- CloudShellを使うことで、AWS CLI、認証情報、OS差異によるつまずきを減らします。
- AWS環境を作らないローカル演習やワークシート編集は、受講者がAWS環境を持っていない場合も進められるよう、Windows前提で案内することがあります。
...
1. 上の教材一覧で対象セクションとハンズオンリソースを確認する。
2. このREADMEと対象ディレクトリのREADMEを最後まで読む。
3. AWS操作を伴う場合は、CloudShellを開き、リージョン、Stack名、Prefix、通知先メール、削除予定時刻を記録する。
4. [`s10-learning-safety-troubleshooting/cost_safety_checklist.md`](s10-learning-safety-troubleshooting/cost_safety_checklist.md) を作業前に記入する。
...
1. [`s3-monitoring-foundation-prep/README.md`](s3-monitoring-foundation-prep/README.md) で作成物と削除条件を確認する。
2. [`s3-monitoring-foundation-prep/setup_commands.md`](s3-monitoring-foundation-prep/setup_commands.md) に沿って作成前確認とテンプレート検証を行う。
3. 実際にAWSリソースを作成する場合は、[`s3-monitoring-foundation-prep/cloudformation/cloudwatch_sre_monitoring_lab.yaml`](s3-monitoring-foundation-prep/cloudformation/cloudwatch_sre_monitoring_lab.yaml) だけを使う。
4. サンプルログ、Custom Metrics、Dashboard、Alarm、SNS、Runbookリンクの役割を整理する。
5. [`s3-monitoring-foundation-prep/logs_insights_queries.md`](s3-monitoring-foundation-prep/logs_insights_queries.md) と [`s3-monitoring-foundation-prep/validation_checklist.md`](s3-monitoring-foundation-prep/validation_checklist.md) で初期状態を確認する。
6. 演習後は [`s3-monitoring-foundation-prep/cleanup_commands.md`](s3-monitoring-foundation-prep/cleanup_commands.md) で削除確認を行う。

### Section 8 SLI/SLO演習

1. [`s8-error-budget-burn-rate/error_budget_sheet.csv`](s8-error-budget-burn-rate/error_budget_sheet.csv) にSLO目標、測定期間、失敗数を入力する。
2. [`s8-error-budget-burn-rate/burn_rate_alarm_design.md`](s8-error-budget-burn-rate/burn_rate_alarm_design.md) に短期窓、長期窓、重要度、通知先、Runbookを記入する。
3. CloudWatch AlarmやApplication Signals SLOを実作成する場合は、料金と削除条件を事前確認する。

### Section 9 Runbook演習

1. [`s9-runbook-first-response/five_minute_first_response_runbook.md`](s9-runbook-first-response/five_minute_first_response_runbook.md) に通知直後の確認事項を書く。
2. [`s9-runbook-first-response/fifteen_minute_investigation_runbook.md`](s9-runbook-first-response/fifteen_minute_investigation_runbook.md) にLogs Insights、Dashboard、Application Signalsの調査順を書く。
...
- AccessDeniedが出る場合は [`s10-learning-safety-troubleshooting/access_denied_triage_sheet.md`](s10-learning-safety-troubleshooting/access_denied_triage_sheet.md) に主体、Action、Resource、Conditionを分解します。
- CloudFormation rollbackが起きた場合は [`s10-learning-safety-troubleshooting/rollback_event_reading_sheet.md`](s10-learning-safety-troubleshooting/rollback_event_reading_sheet.md) で最初の失敗イベントを確認します。
- 何を実行したか分からなくなった場合は [`s10-learning-safety-troubleshooting/evidence_capture_template.md`](s10-learning-safety-troubleshooting/evidence_capture_template.md) に時刻、リージョン、期待結果、実結果を記録します。
- 削除確認は [`s10-learning-safety-troubleshooting/cleanup_checklist.md`](s10-learning-safety-troubleshooting/cleanup_checklist.md) を使い、確認できないリソースを残したまま作業完了にしません。

## Public Repository

このリポジトリは、Udemy講座の受講者向けハンズオン資材を配布するための公開リポジトリです。まずこの [`handson/README.md`](README.md) を読み、必要なセクションのREADMEとワークシートへ進んでください。
